...
TextWindowClass::TextWindowClass(HINSTANCE hInstance, HWND hParent) : hInstanceHandle{ hInstance }, hParentHandle{ hParent }, lpszClassReg{ L"STATIC" }, hWndHandle{ NULL } {
	return;
}

TextWindowClass::~TextWindowClass() {
...
ULONG64 TextWindowClass::CombinePosArgs(INT16 nX, INT16 nY, INT16 nWidth, INT16 nHeight) {
...
ULONG64 TextWindowClass::CombineStyleArgs(UINT32 nArgs, UINT32 nExArgs) {
...
void TextWindowClass::GenerateWindow(ULONG64 nStyleArgsCombined, ULONG64 nPosCombined, LPCWSTR lpszText) {
...
void TextWindowClass::SetFont(HFONT hNewFont) {
	SendMessage(hWndHandle, WM_SETFONT, (WPARAM)hNewFont, TRUE);
	return;
}

HFONT TextWindowClass::GetFont() {
	return (HFONT)SendMessage(hWndHandle, WM_GETFONT, NULL, NULL);
}

void TextWindowClass::DisplayWindow(INT nCmdShow) {
...
LPCWSTR& TextWindowClass::GetClassReg() {
	return lpszClassReg;
}

HWND& TextWindowClass::GetWndHandle() {
	return hWndHandle;
}

HWND& TextWindowClass::GetParentHandle() {
	return hParentHandle;
}

HINSTANCE& TextWindowClass::GetInstanceHandle() {
